...
export const _devices = [...Array(24)].map((_, index) => ({
  id: _id(index),
  org: _company(index),
  dept: ['Engineering', 'Manufacturing', 'Quality', 'Research'][index % 4],
  room: `Room-${(index % 10) + 1}`,
  line: `Line-${(index % 5) + 1}`, 
  display_name: _fullName(index),
  u_id: _id(index),
  device_type: [
    'Temperature Sensor',
    'Humidity Sensor', 
  ...
  firm_ver: `v${Math.floor(index/8) + 1}.${index % 8}.${index % 5}`,
  isVerified: _boolean(index),
  avatarUrl: `/assets/images/avatar/avatar-${index + 1}.webp`,
  status: index % 4 ? 'active' : 'offline',
}));
// ----------------------------------------------------------------------
